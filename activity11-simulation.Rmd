...
norm_samp_30 <- tibble(
  values = rnorm( 30, 50, 10)
)
mean(norm_samp_30$values)
sd(norm_samp_30$values)

```
Since the sample size decreased, the standard deviation increased.
...
mean(rand_exp_1000$values)
mean(rand_exp_30$values)
sd(rand_exp_1000$values)
sd(rand_exp_30$values)

```
For these exponential distribution samples, the sample with the larger n had a higher mean and standard deviation. As expected for an exponential distribution, the mean and standard deviations per sample are roughly equal.
